-- https://sql.telemetry.mozilla.org/queries/48383/source#130607
 WITH population AS
  (SELECT subsession_start,
  ...
  (SELECT subsession_start,
   ...
   FROM population
   WHERE days_since_creation = 0
   GROUP BY 1) -- The total count of first session profiles for each date

SELECT population.subsession_start AS date,
       days_since_creation AS day_number,
       cardinality(merge(hll)) AS value,
       total
FROM population
JOIN population_agg ON population.subsession_start = population_agg.subsession_start
WHERE days_since_creation > 0
GROUP BY 1,
         2,
         4
